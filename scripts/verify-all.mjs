...
//
// ⚠️ **改代码时请用 --fast，收尾/提交前再跑全量**（2026-09-14 实测：全量 **36.7 秒**，
//    其中检查器自检 32.6 秒；--fast 只要 **4.3 秒**）。
//    （自检在 2026-09-14 改成 6 片并发之前是 94.3 秒 —— 全量那会儿要 101.6 秒。）
//    这条不是"偷懒的口子"：自检证明的是"闸门还抓不抓得住已知错误"，
//    它**不随你每次保存而变化** —— 中间循环每改一行都跑它，只会把人逼去用 `--no-verify`，
//    那才是真的什么都没了。**收尾必须跑全量**（提交钩子也会替你跑一次）。
//
// 退出码（三档，不许压成一档）:
//   0 = 全部通过
//   1 = 有检查**真的失败**（内容/行为有问题）
...
const steps = [
  ...
  },
  {
    ...
    // ⚠️ `--parallel`：61 条用例里有 20 条要起 PowerShell，串行实测 92.9 秒；
    //    用例之间彼此独立（各自一份副本、各自 spawn）⇒ 分片并发跑到 34 秒（6 片，2026-09-14 实测）。
    //    动控制台代码页的那条由父进程**单独串行**跑（`chcp` 改的是整个控制台，并行会互相污染）。
    cmd: [node, "scripts/self-test.mjs", "--parallel"],
    skip: FAST,
    skipReason: "--fast",
  },
];
